...
#
# This is based on chainer examples wavenet utils.py in <https://github.com/chainer/chainer/tree/master/examples/wavenet>
# Please see LICENSE-chainer.txt in the docs folder regarding to Chainer license.
#
## Change:
#  Date: Dec. 2018
#        add weight martix between index as class Preprocess output
#
# Check version
#  Python 3.6.4
#  numpy 1.14.0 
...
class Preprocess(object):
    def __init__(self, sr, n_fft, hop_length, n_mels, top_db,
        ...
        '''
        add initial preparation to make weight matrix between index
        '''
        # make base [dim_input-1, ..,2,1,0,1,2,...,dim_input-1]
        fwd = numpy.arange(0, self.quantize, dtype=numpy.float32)  / self.quantize
        rwd = numpy.arange(self.quantize - 1,0, -1, dtype=numpy.float32) / self.quantize
        self.base = numpy.concatenate([rwd, fwd])
    ...
    def __call__(self, path):
        ...
        '''
        add make weight matrix between index
        '''
        weight_matrix = numpy.zeros( (self.quantize, quantized.shape[0] -1 ,1), dtype=numpy.float32)  # (256, length, 1)
        for l, index0 in enumerate(quantized[1:].flatten()):
            weight_matrix[:,l,0] = self.base[ self.quantize - 1 - index0 : self.quantize - 1 - index0 + self.quantize]

        return one_hot[:, :-1], spectrogram, weight_matrix, quantized[1:]
